...
struct ChaosModule : Module {
	...
	ChaosModule() {
		...
		configParam(GRAVITY_PARAM, 0.01f, 4.f, 1.0f, "timewarp", "x");
	...
	void kickPendulums() {
		p0.theta = 2.5f + (random::uniform() * 0.1f);
		p1.theta = 0.25f + (random::uniform() * 0.1f);
	}
	...
	void process(const ProcessArgs& args) override {
		...
		if (frame % 4 == 0) {
			...
			clamp(length_ratio, 0.01f , 0.99f);
			...
			p0.acc = p0_numerator / p0_denominator;
			...
			p1.acc = p1_numerator / p1_denominator;
			...
			outputs[POLY_CHAOS_OUTPUT].setVoltage(p1.y * VOLT_FIVE, 1);
			...
			outputs[POLY_CHAOS_OUTPUT].setChannels(6);
...
struct ChaosWidget : ModuleWidget {
	ChaosWidget(ChaosModule* module) {
		...
		addInput(createInputCentered<PJ301MPort>((Vec(col02_x, row02_y)), module, ChaosModule::RATIO_IN));
		addInput(createInputCentered<PJ301MPort>((Vec(col03_x, row02_y)), module, ChaosModule::DAMPING_IN));
	...
	void appendContextMenu(ui::Menu *menu) override {
		...
		menu->addChild(construct<ChaosModuleItem>(&MenuItem::text, "Kick", &ChaosModuleItem::module, module));
	}
};
